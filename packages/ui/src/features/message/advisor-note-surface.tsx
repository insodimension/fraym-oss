...
export function AdvisorNoteSurface({ fields }: { readonly fields: AdvisorNoteFields }) { return <MessageSurface detail={<ul>{fields.notes.map((note, index) => <li key={index}><strong>{note.severity}</strong> {note.message}</li>)}</ul>} label="Review" tone={fields.notes.some((note) => note.severity === "blocker") ? "danger" : "warning"} />; }
