...
export function LiquidGlassSurface({ variant = "frosted", settings, radius = 16, className, style, children }: LiquidGlassSurfaceProps) {
  ...
  useEffect(() => {
    const root = rootRef.current; const canvas = canvasRef.current; if (!root || !canvas || !field) return;
    let renderer: LiquidGlassRenderer; try { renderer = new LiquidGlassRenderer(canvas, field.source, merged); } catch { return; }
  ...
  useEffect(() => rendererRef.current?.setAnimating(field?.animating ?? false), [field?.animating]);
