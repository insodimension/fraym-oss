...
//
// Ported (verbatim physics/lighting model) from `ogtirth/liquidglass-oss`
// (https://gitlab.com/ogtirth/liquidglass-oss), MIT-licensed, which itself
// adapts `ybouane/liquidglass`. See THIRD_PARTY_NOTICES in the repo root.
//
// The fragment shader renders ONE rounded-rect glass lens per draw:
//   · rrSDF + lensHeight build a spherical-cap height field for the rounded
//     rectangle, whose gradient is the surface normal.
//   · The normal drives screen-space REFRACTION (samples u_bg displaced toward
//     the lens interior), CHROMATIC ABERRATION (spectralSample fans the sample
//     across a 7-band spectrum), FRESNEL + SPECULAR rim light, and an edge
//     catch-light stroke.
//   · u_bg is the ambient field texture (an image, video, or — in Fraym — the
//     live procedural <canvas> field). u_bgScale/u_bgOffset map the lens's
//     on-screen rect into the background element's rect so the glass refracts
//     exactly what sits behind it.
// Uniforms are documented inline in the renderer's draw().
