...
export const renderEdit: ToolRenderer = call => {
  ...
  return { label: "Edit", badges: <code className="fraym-tool-head__target">{path}</code>, stat: `+${added} −${deleted}`, status: call.status, kind: "edit", bodyVariant: "diff", defaultOpen: call.status === "failed", body: <EditToolRenderer {...call} /> };
};
