...
export function CommandTagProvider({ recipes = NO_RECIPES, children }: CommandTagProviderProps) { const resolver = useMemo<CommandTagResolver>(() => { const map = new Map(recipes.map(recipe => [`/${recipe.id}`, { label: recipe.name ?? recipe.id, ...(recipe.icon ? { icon: recipe.icon } : {}) }])); return token => { const known = map.get(token); if (known) return known; const skill = /^\/skill:([\w-]+)$/.exec(token)?.[1]; return skill ? { label: skill, icon: "spark" } : null; }; }, [recipes]); return <CommandTagContext.Provider value={resolver}>{children}</CommandTagContext.Provider>; }
